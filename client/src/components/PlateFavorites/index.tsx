...
export const PlateFavorites = () => {
  ...
  const favoriteIsLoading = useTypeSelector(
    (state) => state.favoriteItems.isLoading
  );
  const { id } = useAuth();
  useEffect(() => {
    if (id != null) {
  ...
  return (
    <Box
      data-testid="plate-favorite"
      className={usePlateFavoriteStyle.classes.cardsFavorites}
    >
      <Typography className={usePlateFavoriteStyle.classes.titleFavorite}>
        Избранное:
      </Typography>
