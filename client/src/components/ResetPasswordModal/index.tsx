...
export const ResetPasswordModal = ({
  ...
  const handleClickSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!isValidEmail(email)) {
      ...
      sendPasswordResetEmail(auth, email, {
        url: process.env.REACT_APP_REDIRECT_PATH || "",
      });
  ...
  const useResetPasswordModalStyle = ResetPasswordModalStyle({
    Pallete: pallete,
  });

  return (
    <Modal
      className={useResetPasswordModalStyle.classes.modal}
      open={open}
      onClose={handleClose}
    >
      <Box
        onSubmit={handleClickSubmit}
        className={useResetPasswordModalStyle.classes.formReset}
        component="form"
      >
        <Typography className={useResetPasswordModalStyle.classes.title}>
          Введите вашу почту
        </Typography>
        <TextField
          required
