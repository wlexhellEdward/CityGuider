...
export const ContainerInputsForm = () => {
  ...
  const handlePasswordChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    dispatch(
      setFormValidity({
        ...
        error,
        colorsOfProgres:
          strength < 35 ? "crimson" : strength < 70 ? "orange" : "green",
      })
    );
  };
  const handleConfirmChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    dispatch(
      setFormValidity({
        ...formValidity,
        confirm: confirm === formValues.password,
      })
    );
  };
  ...
  return (
    <Box className={useRegisterFormStyle.classes.containerInputsForm}>
      ...
      <LinearProgress
        variant="determinate"
        value={formValidity.strength}
        sx={{ "&>*": { backgroundColor: formValidity.colorsOfProgres } }}
      />
      <TextField
        required
        fullWidth
