...
import {
  addFavoriteCard,
  deleteFavoriteCard,
} from "@/utils/firebaseServices/favorites/favoriteService.ts";
...
const CardPlace = ({ place }: CardPlaceProps) => {
  ...
  const { id } = useAuth();
  const userLocation = useTypeSelector(
    (state) => state.currentPosition.position
  );
  const favoriteItems = useTypeSelector(
    (state) => state.favoriteItems.favoriteItems
  );
  const destination = {
  ...
  const handleClickRoute = async () => {
    ...
    dispatch(
      setRouteInfo({ directionsRenderer, distanceTotal, placeLocation, time })
    );
  };
    ...
    return (
      <Card className={useCardPlaceStyle.classes.cardPlace}>
        ...
        <CardContent className={useCardPlaceStyle.classes.cardContent}>
          <Typography className={useCardPlaceStyle.classes.title}>
            {place.name}
          </Typography>
          <Typography className={useCardPlaceStyle.classes.placeAdress}>
            {place.vicinity}
          </Typography>
        </CardContent>
  ...
  return <PopUp text={errors.errorInRenderCardPlace} />;
};
