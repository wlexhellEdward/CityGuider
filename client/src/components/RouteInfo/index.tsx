...
export const RouteInfo = () => {
  ...
  const { distance, distanceTraveled, placeGeometry, time } = useTypeSelector(
    (state) => state.map.travelInfo
  );
  const directionsRenderer = useTypeSelector(
    (state) => state.map.directionsRenderer
  );
  const center = useTypeSelector(
    (state) => state.currentPosition.humanPosition
  );
  ...
  return (
    <Container
      ...
      className={useRouteInfoStyle.classes.routeContainer}
    >
      <Box>
        <LinearProgress
          className={useRouteInfoStyle.classes.progressBar}
          variant="determinate"
          value={progressTravel}
        />
        <Button
          data-testid="route-button-close"
          className={useRouteInfoStyle.classes.buttonClose}
          onClick={() => handleClickDeleteTravel()}
        >
          X
        </Button>
      </Box>
      <Box>
        <Typography className={useRouteInfoStyle.classes.routeValue}>
          {distanceRemaining} км
        </Typography>
        <Typography
          data-testid="route-remaing-time"
          className={useRouteInfoStyle.classes.routeValue}
        >
          {time}
        </Typography>
      </Box>
      <Box>
        <Typography className={useRouteInfoStyle.classes.routeValueDescription}>
          дистацния
        </Typography>
        <Typography className={useRouteInfoStyle.classes.routeValueDescription}>
          примерное время
